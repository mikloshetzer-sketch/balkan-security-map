...
def overall_from_dimensions(dim_levels: Dict[str, str], dim_scores: Dict[str, float], incident_count: int) -> str:
    """
    Javított overall logika:
    - 1 elevated -> elevated
    - tense csak többdimenziós vagy erősebb helyzetben
    - critical csak ritka esetben
    """
...
def regional_overall_from_countries(country_rows: List[Dict[str, Any]]) -> str:
    """
    Régiós overall ne legyen túl agresszív.
    """
...
def feature_to_incident(feature: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # Természeti és riasztási források fékezése
    if source == "USGS":
        score *= 0.40
    elif source == "GDACS":
        score *= 0.55
...
def main() -> int:
    ...
    output = {
        ...
        "method_version": "risk_snapshot_v2_1",
        "sources": {
